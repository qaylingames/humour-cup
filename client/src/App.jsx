...
function App() {
  ...
  // Automatically sync the public scenario form language with the app language
  const [pubScenario, setPubScenario] = useState('');
  const [pubLang, setPubLang] = useState(appLang);
  const [pubCategory, setPubCategory] = useState('All Ages'); // State persists until refreshed
  ...
  // Sync pubLang if they change the app language from the top menu
  useEffect(() => {
    setPubLang(appLang);
  }, [appLang]);
    ...
    <div onClick={unlockAudio} style={styles.appWrapper}>
      <style>{`
        ...
        .animated-logo-main {
           ...
           user-select: none;
           margin-bottom: 20px;
           margin-top: 30px; /* Pushed down slightly */
        }
      `}</style>
      ...
      {!room && !showVault && (
        <div style={styles.translateWrapper}>
          ...
          {/* Changed standard names to Short Codes to save space on mobile */}
      ...
      {/* --- REVERTED TO SINGLE PNG LOGO WITH ANIMATION --- */}
...
const styles = {
  ...
  /* --- MOBILE FIXES FOR TRANSLATE BUTTON --- */
  translateWrapper: { position: 'absolute', top: '10px', left: '10px', zIndex: 1000, display: 'flex', alignItems: 'center', gap: '4px', background: '#fff', padding: '6px 10px', borderRadius: '50px', border: '3px solid #1a1a1a', boxShadow: '2px 2px 0px #1a1a1a' },
